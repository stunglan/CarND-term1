...
def generate_arrays_from_file(path):
    ...
    while 1:
        ...
        for line in f:
            ...
            line = line.split(',') # #center,left,right,steering,throttle,brake,speed

            for i in range(3):
                # center,left,rigth picture
                if line[i].strip():
                    X[batchIndex] = read_image(path+'/'+line[i].strip())
                    angle = float(line[3])
                    if i == 1: # left
                        angle += 0.2
                    elif i == 2: # right
...
history = mymodel.fit_generator(generate_arrays_from_file(data_dir),  samples_per_epoch=256*300, nb_epoch=5, verbose=1)
